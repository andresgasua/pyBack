...
class MeasurementList(AHMResource):
    ...
    def get(self):
        ...
        """
        data = [measurement.to_dict() for measurement in Measurement.objects]  # pylint: disable=no-value-for-parameter
        return data, 200
    ...
    def post(self):
        ...
        user = User.objects(email='andresgamboas@gmail.com').first()
    ...
    def delete(self):
        ...
        """
        records_deleted = Measurement.objects.delete()
        return {'records_deleted': records_deleted}, 200
...
class UserList(AHMResource):
    def __init__(self):
        super().__init__(field_required=True)

    @swag_from('docs/user_list.yml', methods=['GET'])
    def get(self):
        """
        Get user
        """
        data = [user.to_dict() for user in User.objects]  # pylint: disable=no-value-for-parameter
        return data, 200
